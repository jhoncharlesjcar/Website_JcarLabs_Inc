// ⚠️  SERVIDOR LEGACY — Solo para referencia y comparación visual.
// El build de Astro usa src/server/http.mjs (via scripts/serve.mjs).
// No ejecutar como servidor de producción de la migración Astro.
...
Object.defineProperty(window.URL, 'revokeObjectURL', { value: OriginalURL.revokeObjectURL });
    ...
    if (err.code === 'ENOENT') {
      ...
      try {
        const errorPage = path.join(__dirname, '404', 'index.html');
